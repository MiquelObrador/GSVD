...
print(f"Superweight patterns: {SUPERWEIGHTS}")
...
for name, module in model.named_modules():
    if isinstance(module, nn.Linear) and name != "lm_head":
        # skip any layer whose name contains any of the substrings provided by --superweight
        if len(SUPERWEIGHTS) > 0 and any(sw in name for sw in SUPERWEIGHTS):
            print(f"Excluding superweight layer from compression list: {name}")
            continue
...
model_save = {
    'model_state_dict': model.state_dict(),
    'compressed_layers': completed_layers
}
